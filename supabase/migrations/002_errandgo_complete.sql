...
-- It creates tables in dependency order and reconciles partially-created older tables.
...
create table if not exists public.categories (
  id uuid primary key default gen_random_uuid(), name text not null unique, icon text, created_at timestamptz not null default now()
);
...
create table if not exists public.errands (
  ...
  scheduled_at timestamptz, distance_km numeric(10,2), accepted_at timestamptz,
...
create table if not exists public.applications (
  ...
  message text, proposed_price numeric(18,2),
  status text not null default 'pending' check (status in ('pending','accepted','rejected','withdrawn')),
  created_at timestamptz not null default now(), unique(errand_id, runner_id)
);

create table if not exists public.conversations (
  id uuid primary key default gen_random_uuid(), errand_id uuid references public.errands(id) on delete cascade,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists public.messages (
  ...
  attachment_url text, created_at timestamptz not null default now(), read_at timestamptz
);

create table if not exists public.notifications (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
  title text not null, body text not null, type text default 'system', data jsonb not null default '{}'::jsonb,
  read boolean not null default false, created_at timestamptz not null default now()
);

create table if not exists public.transactions (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
  errand_id uuid references public.errands(id) on delete set null, type text not null, amount numeric(18,2) not null,
  currency text not null default 'NGN', reference text unique, status text not null default 'pending',
  metadata jsonb not null default '{}'::jsonb, created_at timestamptz not null default now()
);

create table if not exists public.reviews (
  id uuid primary key default gen_random_uuid(), errand_id uuid not null references public.errands(id) on delete cascade,
  ...
  rating integer not null check (rating between 1 and 5), comment text, created_at timestamptz not null default now(), unique(errand_id, reviewer_id)
);

create table if not exists public.saved_errands (
  user_id uuid not null references public.profiles(id) on delete cascade, errand_id uuid not null references public.errands(id) on delete cascade,
  created_at timestamptz not null default now(), primary key (user_id, errand_id)
);

create table if not exists public.disputes (
  id uuid primary key default gen_random_uuid(), errand_id uuid not null references public.errands(id) on delete cascade,
  opened_by uuid references public.profiles(id) on delete cascade, reason text, description text,
  status text not null default 'open', resolution text, created_at timestamptz not null default now(), resolved_at timestamptz
);

create table if not exists public.support_tickets (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
  subject text not null, message text not null, status text not null default 'open', created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.withdrawals (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
  amount numeric(18,2) not null check (amount > 0), currency text not null default 'NGN', bank_name text, account_name text,
  account_number text, provider_reference text, status text not null default 'pending', created_at timestamptz not null default now(), processed_at timestamptz
);

-- Reconcile tables that may already exist from an earlier ErrandGo schema.
...
-- If an older disputes table used user_id, migrate it into opened_by where possible.
do $$
begin
...
-- Replace policies from previous attempts.
do $$
declare p record;
begin
...
returns public.errands language plpgsql security definer set search_path=public as $$
...
begin
  ...
  where id=p_errand_id and (customer_id=auth.uid() or runner_id=auth.uid()) returning * into r;
